...
<div class="wrap vb-wrap"> 
	...
	<form id="wplm_report_form" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?> method="get">
		<input type="hidden" name="post_type" value="poll" />
		<input type="hidden" name="page" value="wplm_reports" />
		<select name="p">
			<?php
			echo sprintf('<option value="%d">%s</option>', '', __('Select a Poll', WPLM_TEXT_DOMAIN ) );		
	...
	<div id="report-<?php echo $poll_id; ?>" class="report_container" > 
		...
		<?php do_action( 'wplm_admin_action_before_report', $poll_id ); ?>
		<?php do_action( 'wplm_admin_action_report', $poll_id ); ?>
		<?php do_action( 'wplm_admin_action_after_report', $poll_id ); ?>
